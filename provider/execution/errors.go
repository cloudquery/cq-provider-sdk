...
func defaultErrorClassifier(_ schema.ClientMeta, resourceName string, resource *schema.Resource, err error) diag.Diagnostics {
	...
	if strings.Contains(err.Error(), ": socket: too many open files") {
		...
		return FromError(err, diag.WithResourceName(resourceName), WithResource(resource), diag.WithSummary(fdLimitMessage), diag.WithType(diag.THROTTLE), diag.WithSeverity(diag.WARNING))
	}
	return nil
}
...
func FromError(err error, opts ...diag.BaseErrorOption) diag.Diagnostics {
	...
	case diag.Diagnostics:
		return ti
	default:
		e := diag.NewBaseError(err, diag.RESOLVING, opts...)
