			...
			ExpectedFetchResponses: []*cqproto.FetchResourcesResponse{
				{
					...
					Summary: cqproto.ResourceFetchSummary{
						...
						ResourceCount: 0,
						Diagnostics:   diag.Diagnostics{diag.NewBaseError(errors.New("bad error"), diag.RESOLVING, diag.WithResourceName("bad_resource_ignore_error"), diag.WithSeverity(diag.IGNORE), diag.WithSummary(`table "bad_resource_ignore_error" resolver ignored error: bad error`))},
					},
				}},
			ExpectedError: nil,
			MockStorageFunc: func(ctrl *gomock.Controller) *mock.MockStorage {
