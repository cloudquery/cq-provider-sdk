...
func TestExecutionData_ResolveTable(t *testing.T) {
	...
	t.Run("inject fields into execution", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, []string{"name", "name_no_prefix", "prefix_name", "cq_id", "cq_meta", "cq_fetch_date", "injected_field"}, expectedResource.columns)
		assert.Equal(t, []interface{}{"other", "name_no_prefix", "prefix_name", expectedResource.cqId, expectedResource.Get("cq_meta"), exec.executionStart, 1}, values)
	})
