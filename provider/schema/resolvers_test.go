...
func TestPathResolver(t *testing.T) {
	...
	resource := NewResourceData(pathTestTable, nil, testStruct{Inner: innerStruct{Value: "bla"}, Value: 5, unexported: false}, nil, time.Now())
...
func TestDateTimeResolver(t *testing.T) {
	...
	resource := NewResourceData(dateTestTable, nil, testDateStruct{Date: "2011-10-05T14:48:00.000Z"}, nil, time.Now())
	...
	resource = NewResourceData(dateTestTable, nil, testDateStruct{Date: "03 Jan 06 15:04 EST"}, nil, time.Now())
	err = r2(context.TODO(), nil, resource, Column{Name: "date"})
	assert.Nil(t, err)
	...
	resource = NewResourceData(dateTestTable, nil, testDateStruct{Date: "2011-10-05"}, nil, time.Now())
...
func TestNetResolvers(t *testing.T) {
	...
	for _, r := range netTests {
		resource := NewResourceData(networkTestTable, nil, r, nil, time.Now())
...
func TestTransformersResolvers(t *testing.T) {
	...
	resource := NewResourceData(TransformersTestTable, nil, testTransformersStruct{Int: 10, Float: 10.2, String: "123", BadFloat: "10,1"}, nil, time.Now())
...
func TestUUIDResolver(t *testing.T) {
	...
	resource := NewResourceData(UUIDTestTable, nil, testUUIDStruct{UUID: "123e4567-e89b-12d3-a456-426614174000", BadUUID: "123e4567-e89b-12d3-a456-4266141740001"}, nil, time.Now())
