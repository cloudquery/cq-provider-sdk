...
func (p PgDatabase) Insert(ctx context.Context, t *schema.Table, resources schema.Resources) error {
	...
	if err != nil {
		return diag.NewBaseError(err, diag.ERROR, diag.DATABASE, nil, t.Name, "bad insert SQL statement created", fmt.Sprintf("SQL statement %s is invalid", s))
	}
	...
	if pgErr, ok := err.(*pgconn.PgError); ok {
		...
		return diag.NewBaseError(err, diag.ERROR, diag.DATABASE, nil, t.Name, fmt.Sprintf("failed to insert to table %s", t.Name), pgErr.Message)
	}
	return diag.NewBaseError(err, diag.ERROR, diag.DATABASE, nil, t.Name, err.Error(), "")
}
